<script lang="ts">
  ...
  import { respectsReducedMotion } from '../../lib/motion';
...
<section class="hero-console">
  <!-- Background Wheel -->
  <div class="wheel-bg" bind:this={wheelWrapper}>
  ...
  <div class="container hero-inner">
    ...
    <div class="hero-content">
      {#if mounted}
        <h1 class="hero-title neon-text-cyan" bind:this={titleEl} in:scale={{ duration: 600, delay: 200, start: 0.95 }}>
          SCHEDULED-DB
        </h1>

        <p class="hero-subtitle text-[var(--ink-secondary)]" bind:this={subtitleEl} in:fly={{ y: 10, duration: 400, delay: 500 }}>
        ...
        <div class="hero-actions" bind:this={buttonsWrapper}>
          <div in:fly={{ y: 10, duration: 400, delay: 700 }}>
            <HudButton variant="primary" onclick={() => window.location.hash = 'quickstart'}>INITIALIZE</HudButton>
          </div>
          <div in:fly={{ y: 10, duration: 400, delay: 800 }}>
            <HudButton variant="ghost" onclick={() => window.location.hash = 'features'}>VIEW DOCS</HudButton>
          </div>
        </div>
      {/if}
    </div>
