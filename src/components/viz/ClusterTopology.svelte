  ...
  <div class="svg-container">
    <svg viewBox="0 0 600 400" width="100%" height="100%">
      {#if inView}
        ...
        <path d="M 300 150 Q 200 150 100 250" fill="none" stroke="var(--border-hud)" stroke-width="1" stroke-dasharray="2 4" />
        <path d="M 300 150 Q 400 150 500 250" fill="none" stroke="var(--border-hud)" stroke-width="1" stroke-dasharray="2 4" />

        <!-- Animated data packets via CSS -->
        <circle r="3" fill="var(--neon-cyan)" style="filter: drop-shadow(0 0 4px var(--neon-cyan));">
          <animateMotion dur="1.5s" repeatCount="indefinite" path="M 300 150 Q 200 150 100 250" />
        </circle>
        <circle r="3" fill="var(--neon-cyan)" style="filter: drop-shadow(0 0 4px var(--neon-cyan));" opacity="0.6">
          <animateMotion dur="1.5s" begin="0.5s" repeatCount="indefinite" path="M 300 150 Q 200 150 100 250" />
        </circle>
        <circle r="3" fill="var(--neon-cyan)" style="filter: drop-shadow(0 0 4px var(--neon-cyan));">
          <animateMotion dur="1.5s" repeatCount="indefinite" path="M 300 150 Q 400 150 500 250" />
        </circle>
        <circle r="3" fill="var(--neon-cyan)" style="filter: drop-shadow(0 0 4px var(--neon-cyan));" opacity="0.6">
          <animateMotion dur="1.5s" begin="0.5s" repeatCount="indefinite" path="M 300 150 Q 400 150 500 250" />
        </circle>
      {/if}
